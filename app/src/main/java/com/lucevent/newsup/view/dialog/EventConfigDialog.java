...
public class EventConfigDialog implements DialogInterface.OnClickListener {
    ...
    private View.OnClickListener onScheduleDownload = new View.OnClickListener() {
        ...
        public void onClick(View v)
        {
            ...
            Download s = mScheduleManager.getSpecialSchedule(mEvent.code);

            Intent intent = new Intent(mContext, DownloadScheduleEditorActivity.class);
            if (s == null)
                intent.putExtra(AppCode.ACTION, DownloadScheduleEditorActivity.ACTION_CREATE_EVENT);
            else {
                intent.putExtra(AppCode.ACTION, DownloadScheduleEditorActivity.ACTION_MODIFY_EVENT);
