...
import com.lucevent.newsup.data.util.Reader;
...
public abstract class NewsReader extends Reader {
	...
	private final HashMap<Integer, Integer> tagMap;
	...
	@Override
	public NewsArray readRssHeader(String rss_link, int site_code, int section_code)
	{
		org.jsoup.nodes.Document doc = getDocument(rss_link);
		if (doc == null) {
			return new NewsArray();
		}
		if (!rss_link.equals(doc.baseUri())) {
			System.out.println("\n[diff]\n[" + rss_link + "]\n[" + doc.baseUri() + "]");
		}
	...
	// Only development module
	public org.jsoup.nodes.Document read(String url)
	{
